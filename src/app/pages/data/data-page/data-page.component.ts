...
export class DataPageComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  public stationsUrl = computed(() =>
    this.apiService.buildUrl(
      this.stationsApiBaseUrl,
      this.route.snapshot.data['apisConfig'].get('stations'),
    ),
  );
  public parametersUrl = computed(() =>
    this.apiService.buildUrl(
      this.stationsApiBaseUrl,
      this.route.snapshot.data['apisConfig'].get('parameters'),
    ),
  );
  ...
  public timeserieUrl = computed(() =>
    this.apiService.buildUrl(
      this.stationsApiBaseUrl,
      this.route.snapshot.data['apisConfig'].get('timeseries'),
    ),
  );
  ...
  constructor() {
    ...
    this.polygonMeanApiBaseUrl = this.apiService.buildUrl(
      this.route.snapshot.data['apisConfig'].get('baseUrl'),
      this.route.snapshot.data['apisConfig'].get('polygonMeanApi'),
    );
    ...
    this.baseLayers = LayerGroup.getAllLayers(this.route.snapshot.data['baseLayers']).filter(
      (l: Layer) => l instanceof TileLayer,
    );
    this.infoLayers = LayerGroup.getAllLayers(this.route.snapshot.data['infoLayers']).filter(
      (l: Layer) => l instanceof WMSLayer,
    );
    this.dataLayers = this.route.snapshot.data['groupedCheckboxes'];
    this._layerCategories = new Map(
      this.route.snapshot.data['layerCategories'].map((c: LayerCategory) => [c.id, c]),
    );
    this._sensorTypes = this.route.snapshot.data['sensorTypes'];
    this._sensorTypes2 = this.route.snapshot.data['sensorTypes2'];

    this.groupedCheckboxes = this.dataLayers.map((v: LayerGroup) =>
      LayerGroupToCheckboxAdapter.convert(v),
    );

    /* Forms init */
    this.baseLayersForm.valueChanges.subscribe((changes: unknown) =>
      this._onBaselayersRadioChange(changes),
    );
    if (this.infoLayers.length > 0)
      this.infoLayers.forEach((l: WMSLayer) =>
        this.infoLayersForm.addControl(l.id, new FormControl(false)),
      );
    Object.keys(this.infoLayersForm.controls).forEach((controlName: string) => {
      const control = this.infoLayersForm.get(controlName);
      if (control)
        control.valueChanges.subscribe((changes: boolean) =>
          this._onInfoLayersCheckboxesChange(controlName, changes),
        );
    });
  ...
  public async setDataFromApi() {
    ...
    try {
      const [stationsPick, allStations, sensorTypes] = await Promise.all([
        this.stationsService.getStationParameters(
          this.stationParametersUrl(),
          this.auth2Service.token(),
        ),
      ...
      this._sensorTypes = this._sensorTypes.filter((s: SensorType) =>
        sensorTypes.some(
          (sensor: Sensor) => s.id === sensor.type || s.id === `${sensor.type}--cumulative`,
        ),
      );
  ...
  private _applyLayersFromQueryParams(params: URLSearchParams): void {
    ...
    if ([...layerIds, ...baseLayerIds, ...infoLayerIds].length === 0) {
      ...
      this.globalStateService.updateAllQueryParams2(
        // Sync query params
        new Map(
          Object.entries({
    ...
    this._currentDataLayers = layers.reduce((acc: Map<string, string[]>, curr: Layer) => {
      return this.layersService.checkLayerCategories(
        curr,
        true,
        acc,
        this._layerCategories,
        !!this.user,
      );
    ...
    if (infoLayers.length > 0)
      this.infoLayersForm.patchValue(
  ...
  public onMapLayerAdded(event: Record<string, unknown>): void {
    ...
    const foundLayer: Layer | undefined = LayerGroup.getAllLayers(this.dataLayers).find(
      (l: Layer) => l.id === id,
    );
    ...
    // Chips
    let iconUrl: string = '';
    if (event['icon'] && event['icon'] instanceof SVGSVGElement)
      iconUrl = Utils.svgElementToImgSrc(event['icon']);
    let chipId: string = '';
    ...
    const colorScale: ColorScale | undefined = this._generateLayerColorScale(
      foundLayer,
      this.baseColorScales,
    );
    if (!colorScale) return;
    this.geojsonLegends.push({
      layerId: foundLayer.id,
      layerLabel: foundLayer.longLabel ?? foundLayer.label,
      unit: foundLayer.legend.altUnit || foundLayer.legend.unit,
      colors: colorScale.colors,
      labels: foundLayer.legend.labels ?? colorScale.calculateTicks(),
      date: !foundLayer.layerType.includes('wms')
        ? (this.globalStateService.getDateFromQueryParams() ?? new Date())
        : undefined,
    });
    queueMicrotask(() => {
  ...
  public onMapClicked(event: Record<string, unknown>) {
    ...
    this.layersService
      .getFeatureInfoWMSLayer(
        layer,
        bbox,
        point as { x: number; y: number },
        size as { width: number; height: number },
        this.wmsLayersDate?.toISOString(),
      )
      .then((info: [string, number][]) => {
        info.forEach(([label, value]: [string, number]) => {
          this._map.openCustomPopup(
            `<p><strong>${label}:</strong> ${layer.multiplier ? Utils.truncateValueByDecimals(value * layer.multiplier, layer.decimals ?? 1) : Utils.truncateValueByDecimals(value, layer.decimals ?? 1)} ${layer.legend && layer.legend.unit ? layer.legend.unit : ''}</p>`,
      ...
      .catch((err: unknown) => {
        this.snackbarsService.createSnackbar(
          err instanceof Error ? err.message : `Errore nel recupero dei dati puntuali del layer`,
          'error',
          true,
        );
      });
  ...
  public onFeatureClicked(event: Record<string, any>): void {
    ...
    this._map.openCustomPopup(
      `<p><strong>${layer.label}:</strong> ${Utils.truncateValueByDecimals(properties['mean_value'] * (layer.multiplier ? layer.multiplier : 1), layer.decimals ?? 1)} ${layer.legend && layer.legend.unit ? layer.legend.unit : ''}</p>`,
      coordinates,
    );
  }
  ...
  private _onBaselayersRadioChange(changes: unknown): void {
    ...
    const layer: TileLayer | undefined = this.baseLayers.find(
      (l: TileLayer) => l.id === changes['baseLayer'],
    );
    if (!layer) return;
      ...
      switch (s.type) {
        case 'platform': {
          const station: StationBase | undefined = this.stations.find(
            (station: StationBase) => station.id === s.id,
          );
          const thresholds: Record<string, number> = {};
          if (station?.thresholdConfig)
            Object.entries(station.thresholdConfig).forEach(([k, v]: [string, number]) => {
              if (Utils.isValidColor(k) && v) thresholds[k] = v;
            });
          const foundSensor: SensorType | undefined = this._sensorTypes.find(
            (sensor) => sensor.id === s.parameter,
          );
          newCharts.push(
            this.stationsService.createChart(
              s,
              this._sensorTypes,
              foundSensor && foundSensor.thresholdKeys ? thresholds : {},
            ),
          );
  ...
  public debounceOnChartParameterChange = Utils.debounce(
    (stationCode: string, chartId: string, formChange: Record<string, string>) =>
      this.onChartParameterChange(stationCode, chartId, formChange),
    200,
  );
  ...
  public async onChartParameterChange(
    ...
    const station: StationBase | undefined = this.stations.find(
      (s: StationBase) => s.id === stationCode,
    );

    if (!initialDate) {
      const sensorType = this._sensorTypes.find((t) => t.id === param);
      initialDate = DateUtils.toDateTimeLocal(
        this.stationsService.getInitialDateOnSensorGap(endingDate, sensorType),
      );
    }
    ...
    this.stationsService
      .updateChart(
        ...
        initialDate,
        endingDate,
        DateUtils.toDateTimeLocal(currentDate),
        station?.thresholdConfig,
        this.auth2Service.token(),
      )
  ...
  public onChartCustomButtonClick(event: unknown[]): void {
    ...
    const csv = CSVUtils.convertTimestampValueArrayToCSV(
      charts.map((v) => v.data),
      ['Data', ...charts.map((v) => v.legend ?? '')],
    );
  ...
  public onParameterSaveClick(): void {
    if (!this.user) return;
    const params = this.globalStateService.getQueryParams2([
      'base',
      'info',
      'layer',
      'date',
    ...
    const snackbarId: string = this.snackbarsService.createSnackbar(
      `Salvataggio preferenze dell'utente in corso...`,
      'loader',
      false,
      'snackbar_user_preferences',
    );
    this.globalStateService
      .saveQueryParams(
        this.createConfigUrl,
        `${this.user.id}_${new Date().getTime()}`,
        `${this.user.id}_preferences`,
        'prod',
      ...
      .then(() => {
        this.snackbarsService.createSnackbar(
          `Preferenze dell'utente salvate con successo.`,
          'success',
          true,
        );
      })
      .catch(() => {
        this.snackbarsService.createSnackbar(
          `Errore nel salvataggio delle preferenze dell'utente.`,
          'error',
          true,
        );
      })
   ...
   */
  public onLayerToggled(
    data: unknown,
    toggleLayer: boolean = true,
    updateUrl: boolean = true,
  ): void {
    if (typeof data !== 'object' || data === null || !('id' in data) || !('isChecked' in data))
      return;
    ...
    if (updateUrl)
      this.globalStateService.updateQueryParam2('layer', this.currentDataLayers.toArray());
    if (
      this.layersService.getLayerCountByCategory(this.currentDataLayers.map, 'data_wms--time') <= 0
    )
      this.wmsLayersDate = undefined;
  }

  private _checkLayerAndRedrawGroupedCheckboxes(
    id: string,
    isChecked: boolean,
    isAuth: boolean,
  ): void {
    ...
    this._currentDataLayers = this.layersService.checkLayerCategories(
      foundLayer,
      isChecked,
      this._currentDataLayers,
      this._layerCategories,
      isAuth,
    );
    this.groupedCheckboxes = this._redrawGroupedCheckboxes(
      this._groupedCheckboxes.map((g) => GroupedCheckboxItem.createFromObject(g.group())),
    );
    this.cdRef.detectChanges();
  }

  private _redrawGroupedCheckboxes(
    groupedCheckboxes: GroupedCheckboxItem[],
  ): GroupedCheckboxItem[] {
  ...
  private _toggleGroupedCheckboxes(
    areDisabled: boolean,
    groupedCheckboxes: GroupedCheckboxItem[],
  ): GroupedCheckboxItem[] {
    const newCheckboxes: GroupedCheckboxItem[] = [];
  ...
  private async _toggleLayersOnMap(
    dataLayers: LayerGroup[],
    currentLayers: string[],
  ): Promise<void> {
    const allLayers = LayerGroup.getAllLayers(dataLayers);

    const toRemove = allLayers.filter(
      (l: Layer) => !currentLayers.includes(l.id) && this._map.haslayer(l.id),
    );
    const toAdd = allLayers.filter(
      (l: Layer) => currentLayers.includes(l.id) && !this._map.haslayer(l.id),
    );
  ...
  private _generateLayerColorScale(
    layer: Layer,
    allBaseColorScales: ColorScaleBase[],
  ): ColorScale | undefined {
    if (!layer.legend) return;

    const baseColorScale: ColorScaleBase | undefined = allBaseColorScales.find(
      (c: ColorScaleBase) => c.id === layer.legend?.colorScaleId,
    );
  ...
  private async _executeAction(layer: Layer, date?: Date): Promise<void> {
    ...
    if (layer.legend) {
      const baseColorScale: ColorScaleBase | undefined = this.baseColorScales.find(
        (c: ColorScaleBase) => c.id === layer.legend?.colorScaleId,
      );
      if (baseColorScale) colorScale = new ColorScale(baseColorScale, layer.legend);
    }

    const snackbarId = this.snackbarsService.createSnackbar(
      `Caricamento layer ${layer.label}`,
      'loader',
      false,
    );

    try {
      this.groupedCheckboxes = this._toggleGroupedCheckboxes(
        true,
        this._groupedCheckboxes.map((g) => GroupedCheckboxItem.createFromObject(g.group())),
      );
      ...
      await command.execute({
        ...
        layer,
        baseUrl:
          layer.action['api'] !== 'polygonmean'
            ? this.apiService.buildUrl(this.stationsApiBaseUrl, this.retentionBridgeUrl, '')
            : this.polygonMeanApiBaseUrl,
      ...
      const isNotFoundTimError: boolean =
        err instanceof Error && err.message.includes('non disponibile per la data selezionata');
      this.snackbarsService.createSnackbar(
        err instanceof Error && isNotFoundTimError
          ? err.message
          : `Errore nel caricamento del layer ${layer.label ?? layer.id}. Riprovare.`,
        isNotFoundTimError ? 'success' : 'error',
        true,
      );
  ...
  public onMapDateChanged(date: Date | undefined): void {
    ...
    if (date)
      this.globalStateService.updateQueryParam2('date', [
        this.globalStateService.toDatetimelocal(date),
      ]);
  ...
  public onMapTimedimensionLayerNotFound(layerId: string) {
    ...
    this.snackbarsService.createSnackbar(
      `Il layer ${layersToRemove[0].label ?? layersToRemove[0].id} non è disponibile alla data selezionata.`,
      'success',
      false,
    );
  }
  ...
  public onMapTimedimensionEvent(event: Record<string, any>): void {
    if (
      !('message' in event) ||
      (!('type' in event) &&
        (event['type'] !== 'success' || event['type'] !== 'loader' || event['type'] !== 'error'))
    )
      return;
  ...
  private _updateMultipleLayers(date: Date | undefined, isReset: boolean = false) {
    ...
    const { withKey: layersToKeep, withoutKey: layersToUpdate } = Utils.splitMapByKey(
      this.currentDataLayers.map,
      'data_wms--time',
    );
    ...
    [...layersToUpdate, ...(isReset ? layersToKeep : [])].forEach((id: string) => {
      const foundLayer: Layer | undefined = LayerGroup.getAllLayers(this.dataLayers).find(
        (l: Layer) => l.id === id,
      );
    ...
    Promise.allSettled(promises).then((results) => {
      const fulfilledIndexes: number[] = results
        .map((r, i) => (r.status === 'fulfilled' ? i : undefined))
        .filter((r) => r !== undefined);
      const fulfilledIds = [...layersToUpdate, ...layersToKeep].filter((_, i) =>
        fulfilledIndexes.includes(i),
      );
      fulfilledIds.forEach((id: string) =>
        this._checkLayerAndRedrawGroupedCheckboxes(id, true, !!this.user),
      );
    });
  }
}
