...
export class PlotlyChartComponent implements AfterViewInit, OnDestroy {
  ...
  private _normalizeData(serie: PlotlyChartData): Partial<Plotly.Data> {
    return {
      x: serie.data.map((v: [number, number | null]) => v[0]),
      y: serie.data.map(() => -10),
      text: serie.data.map((v: [number, number | null]) =>
        v[1] !== null && v[1] !== undefined ? `${String(v[1])} ${serie.unit}` : '',
      ),
      hoverinfo: 'text',
    };
  }
  ...
  private _drawChart(data: PlotlyChartData[]): void {
    ...
    Plotly.newPlot(this.id(), traces, layout, config).then((chart: Plotly.PlotlyHTMLElement) => {
      chart.on('plotly_relayout', (e: Plotly.PlotRelayoutEvent) =>
        this._onChartRelayout(e, data, traces, layout, config),
      );
    ...
    return data.map((serie: PlotlyChartData) => {
      ...
      const trace: Plotly.Data = {
        ...(serie.type === 'scatter' && serie.style && serie.style['marker']
          ...
          : this._parseData({
              ...serie,
              data: this._fillGapData(serie.data as [number, number][]),
            })),
        type: serie.type,
  ...
  private _getLayout(data: PlotlyChartData[]): Partial<Plotly.Layout> {
    ...
    const lastDateShape: Partial<Plotly.Shape> | undefined =
      this._createShapeForLastDateValue(data);
    if (lastDateShape) layout.shapes?.push(lastDateShape);

    const range: [number, number] | undefined =
      this.xRange().length === 2 ? (this.xRange() as [number, number]) : this._getDateRange(data);
    ...
    data.forEach((d: PlotlyChartData) => {
      ...
      (layout as any)[axisName] = {
        title: {
          text: !mainYAxis
            ? d.yLabel && d.unit
              ? `${d.yLabel} (${d.unit})`
              : undefined
            : mainYAxis.yLabel && mainYAxis.unit
              ? `${mainYAxis.yLabel} (${mainYAxis.unit})`
              : undefined,
          font: {
            size: 10,
        ...
        range:
          d.unit && d.unit !== '°'
            ? d.yRange && maxYValue > d.yRange[1]
              ? [d.yRange[0], maxYValue + 10]
              : d.yRange
            : undefined,
        nticks: 20,
      ...
      if (d.type === 'scatter' && d.style && d.style['marker']) {
        ...
        if (typeof xMin[0] === 'number' && typeof xMax[0] === 'number')
          layout.annotations = this._createFakeMarkersAsAnnotations(data, d, xMin[0], xMax[0]);
      }
    });
      ...
      modeBarButtonsToAdd: [
        {
          ...
          click: (gd) => {
            Plotly.toImage(gd, {
              format: 'png',
              height: (gd as any)._fullLayout.height,
              width: (gd as any)._fullLayout.width,
            }).then((url) => {
              this._addTitleToImage(
                url,
                (gd as any)._fullLayout.width,
                (gd as any)._fullLayout.height,
              ).then((newUrl: string) => {
  ...
  private async _addTitleToImage(url: string, width: number, height: number): Promise<string> {
  ...
  private _onChartRelayout(
    ...
    data.forEach((d: PlotlyChartData, i: number) => {
      /** Cumulated */
      if (d.isCumulated) {
        const newTraces: Plotly.Data[] | undefined = this._relayoutCumulatedValues(
          data,
          traces,
          i,
          xRange,
      ...
      if (d.type === 'scatter' && d.style && d.style['marker']) {
        ...
        const annotations = this._createFakeMarkersAsAnnotations(
          data,
          d,
  ...
  private _relayoutCumulatedValues(
    data: PlotlyChartData[],
    traces: Plotly.Data[],
    traceIndex: number,
    ...
    if (!otherData) return;
    const cumulatedValues: [number, number][] = this._calculateCumulatedValue(
      otherData,
      new Date(xRange[0]).getTime(),
      new Date(xRange[1]).getTime(),
    );
  ...
  private _relayoutMarkers(
    serie: PlotlyChartData,
    traces: Plotly.Data[],
    traceIndex: number,
    xRange: [number, number],
  ): Plotly.Data[] | undefined {
    const newValues = this._getVisileDataFromXRange(
      serie.data,
      new Date(xRange[0]).getTime(),
      new Date(xRange[1]).getTime(),
    );
  ...
  private _calculateCumulatedValue(
  ...
  private _getRelayoutXRange(event: Plotly.PlotRelayoutEvent): [number, number] | undefined {
    ...
    const isXMinDate: boolean = Boolean(
      xRange[0] && typeof xRange[0] === 'string' && !isNaN(new Date(xRange[0]).getTime()),
    );
    const isXMaxDate: boolean = Boolean(
      xRange[1] && typeof xRange[1] === 'string' && !isNaN(new Date(xRange[1]).getTime()),
    );
  ...
  private _getVisileDataFromXRange(
    data: [number, number | null][],
    xMin: number,
    xMax: number,
  ): [number, number][] {
    return data.filter(
      (point): point is [number, number] =>
        point[0] >= xMin && point[0] <= xMax && point[1] !== null,
    );
  }
  ...
  private _createFakeMarkersAsAnnotations(
    allData: PlotlyChartData[],
    chartData: PlotlyChartData,
    xMin: number,
    xMax: number,
  ): Partial<Plotly.Annotations>[] {
    const otherData: PlotlyChartData | undefined = allData.find(
      (d) => d.sensor === chartData.style?.['markers']?.['relatedSensor'],
    );
    ...
    return data.map((p: [number, number | null]) => {
      const relatedData: [number, number | null] | undefined = otherData.data.find(
        ([t]) => t === p[0],
      );
      ...
      return {
        ...
        y: -10,
        text:
          relatedData && relatedData[1] && chartData.style
            ? this._getMarkerFromStyle(relatedData[1], chartData.style['markers'])
            : '',
  ...
  private _getMarkerFromStyle(value: number, markers: Record<string, any>): string {
    if (
      !('rules' in markers) &&
      !Array.isArray(markers['rules']) &&
      !('comparisonOperator' in markers['rules']) &&
      !('threshold' in markers['rules']) &&
      !('marker' in markers['rules'])
    )
      return '';
    ...
    for (const rule of markers['rules']) {
      switch (rule['comparisonOperator']) {
        case '<':
          if (value < rule['threshold']) return rule['marker'];
          break;
        case '<=':
          if (value <= rule['threshold']) return rule['marker'];
          break;
  ...
  private _createShapeForLastDateValue(data: PlotlyChartData[]): Partial<Plotly.Shape> | undefined {
    ...
    const lastXValue: number = Math.max(
      ...data.flatMap((v: PlotlyChartData) =>
        v.data
          .map((d: [number, number | null]) => d[0])
          .filter((date) => !isNaN(date) && date !== null),
      ),
    );
  ...
  private _getDateRange(chartData: PlotlyChartData[]): [number, number] | undefined {
    const data = chartData
      .map((c) => c.data)
      .map((v) => v.filter((p): p is [number, number] => p[1] !== null));
